...
import com.example.baibu.running.CurrentLocation;
import com.example.baibu.running.MyAppLication;

import java.util.List;

public class MapUtill {
    private static MapUtill INSTANCE = new MapUtill();
    ...
    public static MapUtill getInstance() {
        return INSTANCE;
    }
    ...
    public void drawHistoryTrack(BaiduMap baiduMap,List<LatLng> points, SortType sortType) {
        ...
        if (points.size() == 1) {
            ...
            animateMapStatus(points.get(0), 20.0f);
            return;
        }
